...
merge m:1 year month using ${path}\Dta\deflate.dta, keep(1 3) keepusing(${deflator}) nogen
keep if year == 2010 | year == 2019
qui gen nmw_start = 299/${deflator}     
compress 

qui gen nmw_start_2 = round(ln(nmw_start/2), 0.01)
qui replace nmw_start = round(ln(nmw_start), 0.01)		

qui gen mincat_start=0
qui replace mincat_start=1 if nmw_start <= min
 
local i=1
 while `i'<=30 {
  qui replace mincat_start=1+`i' if nmw_start> min+(`i'-1)*${step} & nmw_start <=min+`i'*${step}
  local i=`i'+1
 }
 
qui gen mincat_half_start=0
qui replace mincat_half_start=1 if nmw_start_2 <=min

local i=1
while `i'<=30 {
qui replace mincat_half_start=1+`i' if nmw_start_2> min+(`i'-1)*${step} & nmw_start_2 <= min+`i'*${step}
local i=`i'+1
 }
...
replace prw=cumwage         if inccat==52
replace p_prw=p_cumwage     if inccat==52
replace c_prw=c_cumwage     if inccat==52
replace pn_prw=pn_cumwage   if inccat==52
...
save ${path}\Results\rwgt_${deflator}_nominal.dta, replace
